...
const float DIAGONAL_SPEED_FACTOR = 0.8;
...
void process_player() {
	...
	if ((IsKeyDown(RIGHT_BUTTON) || IsKeyDown(LEFT_BUTTON)) && (IsKeyDown(DOWN_BUTTON) || IsKeyDown(UP_BUTTON))) {
		player_pos_x += player_speed * (IsKeyDown(RIGHT_BUTTON) - IsKeyDown(LEFT_BUTTON)) * DIAGONAL_SPEED_FACTOR;
		player_pos_y += player_speed * (IsKeyDown(DOWN_BUTTON) - IsKeyDown(UP_BUTTON)) * DIAGONAL_SPEED_FACTOR;
	} else {
